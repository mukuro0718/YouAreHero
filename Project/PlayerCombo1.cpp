	...
	: PlayerAction()
{
	...
	this->nextAnimation				  = static_cast<int>(Player::AnimationType::COMBO_1);										//次のアニメーション
	this->playTime					  = json.GetJson(JsonManager::FileType::PLAYER)["ANIMATION_PLAY_TIME"][this->nextAnimation];//アニメーション再生時間
	this->firstPlayTime				  = json.GetJson(JsonManager::FileType::PLAYER)["COMBO1_START_ANIM_PLAY_TIME"];				//アニメーションの初期再生時間
	this->cancelableFrame			  = json.GetJson(JsonManager::FileType::PLAYER)["COMBO1_CANCEL_FRAME"];						//キャンセル可能フレーム
	this->startHitCheckFrame		  = json.GetJson(JsonManager::FileType::PLAYER)["COMBO1_START_HIT_CHECK_FRAME"];			//当たり判定開始フレーム
	this->endHitCheckFrame			  = json.GetJson(JsonManager::FileType::PLAYER)["COMBO1_END_HIT_CHECK_FRAME"];				//当たり判定終了フレーム
