...
BossSlashCombo2Action::BossSlashCombo2Action()
	:isClose			(false)
	, ATTACK_TYPE		(static_cast<int>(Boss::AttackType::SLASH_COMBO_2))
	...
	, ROTATE_PLAY_TIME_1(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_ROTATE_PLAY_TIME_1"])
	, ROTATE_PLAY_TIME_2(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_ROTATE_PLAY_TIME_2"])
	, MOVE_PLAY_TIME	(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_MOVE_PLAY_TIME"])
	, STOP_PLAY_TIME	(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_STOP_PLAY_TIME"])
	, STOP_DISTANCE		(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_STOP_MOVE_DISTANCE"])
	, ROTATE_LERP_VALUE	(Gori::Convert(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["ROTATE_LERP_VALUE"]))
	, MAX_INTERVAL		(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_INTERVAL"])
	, SLOW_PLAY_TIME	(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_SLOW_PLAY_TIME"])
	, ACTION_DISTANCE	(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["ACTION_DISTANCE"][this->ATTACK_TYPE])
	, SLOW_LIMIT_TIME	(Singleton<JsonManager>::GetInstance().GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_INTERVAL"])
	...
	, CHECK_STATE		(static_cast<int>(Boss::BossState::ANGRY))
{
	...
	this->attack			= new BossSlashCombo2Attack(this->ATTACK_TYPE);
...
void BossSlashCombo2Action::Update(Boss& _boss)
{
	...
	if (!this->isInitialize)
	{
		...
		const VECTOR POSITION				= _boss.GetRigidbody().GetPosition();						//座標
			  VECTOR positonToTargetVector	= VSub(POSITION, TARGET);									//座標→目標へのベクトル
			  VECTOR targetOffset			= VNorm(VCross(VNorm(positonToTargetVector), Gori::UP_VEC));//目標オフセット
	...
	if (this->nowTotalAnimPlayTime < this->ROTATE_PLAY_TIME_1 ||
		this->nowTotalAnimPlayTime >= this->ROTATE_PLAY_TIME_2)
	{
		isRotation = true;
