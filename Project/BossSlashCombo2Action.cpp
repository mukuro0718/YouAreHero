...
void BossSlashCombo2Action::Update(Boss& _boss)
{
	...
	/*移動処理*/
	{
		...
		bool   isMove = false;																	//移動するか
		{
			...
			if (this->frameCount < json.GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_MOVE_FRAME"])
			{
				isMove = true;
			}
			//フレームカウントが定数以内なら回転フラグを立てる
			if (this->frameCount < json.GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_2_ROTATE_FRAME"] || 
