	...
	: BossAttack()
{
	...
	//35-40 57-62
	this->onHitCheckFrame  = json.GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_1_ON_DO_HIT_PLAY_TIME"];
	this->offHitChekcFrame = json.GetJson(JsonManager::FileType::ENEMY)["SLASH_COMBO_1_OFF_DO_HIT_PLAY_TIME"];
...
void BossSlashComboAttack::Update(const float _playTime)
{
	...
	if (this->isStartHitCheck)
	{
		...
		if (this->frameCount == this->onHitCheckFrame)
		{
			//サウンドエフェクトの再生
		...
		else if (this->frameCount == this->offHitChekcFrame)
		{
