...
/// 初期化
/// </summary>
void BossHurricaneKick::Initialize(GoriLib::Physics* _physics)
{
	/*シングルトンクラスのインスタンスの取得*/
	auto& json = Singleton<JsonManager>::GetInstance();
...
/// 更新
/// </summary>
void BossHurricaneKick::Update(GoriLib::Physics* _physics)
{
	/*シングルトンクラスのインスタンスの取得*/
	auto& json   = Singleton<JsonManager>::GetInstance();
	...
	if (this->isStartHitCheck)
	{
		...
		//当たり判定位置の更新
		VECTOR position = MV1GetFramePosition(enemy.GetModelHandle(), 30);
...
const float BossHurricaneKick::GetRadius()const
{
	auto sphereColiderData = dynamic_cast<GoriLib::ColliderDataSphere*>(this->colliderData);
	return sphereColiderData->radius;
}

void BossHurricaneKick::SetRadius(const float _radius)
{
	auto sphereColiderData = dynamic_cast<GoriLib::ColliderDataSphere*>(this->colliderData);
