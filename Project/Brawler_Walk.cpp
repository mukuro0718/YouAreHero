...
Brawler_Walk::NodeState Brawler_Walk::Update(BehaviorTree& _tree, Character& _chara)
{
	...
	//判定
	BrawlerEnemy::AnimationType animationType = BrawlerEnemy::AnimationType::IDLE;
	if (VSize(newVelocity) > 0.01f)
	{
		if (dot > this->THRESHOLD)
		{
			animationType = BrawlerEnemy::AnimationType::WALK_FRONT;
		}
		else if (dot < -this->THRESHOLD)
		{
			animationType = BrawlerEnemy::AnimationType::WALK_BACK;
		}
		else if (cross.y > this->THRESHOLD)
		{
			animationType = BrawlerEnemy::AnimationType::WALK_LEFT;
		}
		else if (cross.y < -this->THRESHOLD)
		{
			animationType = BrawlerEnemy::AnimationType::WALK_RIGHT;
		}
	}
